...
/**
 * <p>
 * A <code>Result</code> is a data structure representing a result returned by
 * the QA engine.</p>
 *
 * <p>
 ...
 * <li>other elements depending on the granularity of the answer string (e.g.
 * sentence, factoid)</li>
 * </ul></p>
 *
 * <p>
 * This class implements the interfaces <code>Comparable</code> and
...
//			return answer.compareTo(result.getAnswer());  // tie-breaking
    ...
    /**
     * Indicates whether an other result is equal to this one. Two results are
     * considered equal if the answer strings are equal.
     *
    ...
    public boolean extractedWith(String technique) {
        ...
        for (String extractionTechnique : extractionTechniques) {
            if (technique.equals(extractionTechnique)) {
                return true;
            }
        }
    ...
    /**
     * add an extra score to this Result for storage, extra score will not
     * influence sorting
     *
     ...
     * @return the extra score set by the source with the specified name, or 0,
     * if the is no suchh extra score
     */
    public float getExtraScore(String sourceName) {
        if (this.extraScores.containsKey(sourceName)) {
            return this.extraScores.get(sourceName).floatValue();
        } else {
            return 0f;
        }
    }
